...
import entrana from './assets/entraña.jpg';
    ...
    <div style={{ background: '#080706', color: '#d4c5a9', fontFamily: "'Josefin Sans', sans-serif", fontWeight: 300, overflowX: 'hidden', minHeight: '100vh' }}>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .fr-nav-cta { font-family:'Josefin Sans',sans-serif; font-size:10px; font-weight:400; letter-spacing:4px; text-transform:uppercase; color:#b48c3c; border:1px solid rgba(180,140,60,0.5); background:transparent; cursor:pointer; padding:10px 24px; transition:all 0.3s ease; }
        .fr-nav-cta:hover { background:rgba(180,140,60,0.15); color:#e8d5a3; }

        .fr-hero-btn-circle { width:56px; height:56px; border:1px solid rgba(180,140,60,0.5); border-radius:50%; display:flex; align-items:center; justify-content:center; background:rgba(180,140,60,0.1); transition:all 0.4s ease; }
        ...
        .fr-arte-card-img { width:100%; height:420px; object-fit:cover; filter:brightness(0.6) saturate(0.7); transition:filter 0.6s ease, transform 0.7s ease; display:block; }
        .fr-arte-card:hover .fr-arte-card-img { filter:brightness(0.8) saturate(1); transform:scale(1.04); }
        .fr-arte-card-arrow { position:absolute; top:24px; right:24px; width:36px; height:36px; border:1px solid rgba(180,140,60,0); border-radius:50%; display:flex; align-items:center; justify-content:center; background:rgba(8,7,6,0.5); transition:all 0.4s ease; opacity:0; }
        ...
        .fr-menu-item-img { width:100%; height:100%; object-fit:cover; filter:brightness(0.55) saturate(0.6); transition:filter 0.5s, transform 0.6s; display:block; }
        ...
        .fr-exp-block::before { content:''; position:absolute; top:0; left:0; right:0; height:2px; background:linear-gradient(to right,transparent,#b48c3c,transparent); transform:scaleX(0); transition:transform 0.5s ease; }
        .fr-exp-block:hover::before { transform:scaleX(1); }

        .fr-btn-primary { font-family:'Josefin Sans',sans-serif; font-size:10px; font-weight:400; letter-spacing:4px; text-transform:uppercase; color:#080706; background:#b48c3c; border:none; padding:18px 48px; cursor:pointer; transition:all 0.3s ease; }
        .fr-btn-primary:hover { background:#c9a050; }
        .fr-btn-secondary { font-family:'Josefin Sans',sans-serif; font-size:10px; font-weight:400; letter-spacing:4px; text-transform:uppercase; color:#b48c3c; background:transparent; border:1px solid rgba(180,140,60,0.4); padding:18px 48px; cursor:pointer; transition:all 0.3s ease; }
        ...
        .fr-menu-tab { font-family:'Josefin Sans',sans-serif; font-size:10px; font-weight:400; letter-spacing:4px; text-transform:uppercase; color:#6b5c40; background:none; border:none; padding:16px 32px; cursor:pointer; position:relative; transition:color 0.3s ease; }
        .fr-menu-tab::after { content:''; position:absolute; bottom:-1px; left:0; right:0; height:2px; background:#b48c3c; transform:scaleX(0); transition:transform 0.3s ease; }
      ...
      {/* NAV */}
      <nav style={{ position: 'sticky', top: 0, zIndex: 100, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 48px', background: 'rgba(8,7,6,0.92)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(180,140,60,0.2)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{ width: 40, height: 40, border: '1px solid rgba(180,140,60,0.5)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(180,140,60,0.08)', overflow: 'hidden' }}>
            <img src={logo} alt="Capris Bull" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          </div>
          <span style={{ fontFamily: "'Josefin Sans',sans-serif", fontWeight: 100, fontSize: 16, letterSpacing: 6, color: '#e8d5a3', textTransform: 'uppercase' }}>Capris Bull</span>
        </div>
      ...
      <header style={{ position: 'relative', minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', paddingBottom: 80, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0 }}>
          ...
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(8,7,6,0.6) 0%, rgba(8,7,6,0.3) 50%, #080706 100%)' }} />
        </div>
        {/* Grid overlay */}
        ...
        {/* Content grid */}
        <div style={{ position: 'relative', zIndex: 10, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0, maxWidth: 1200, margin: '0 auto', padding: '0 80px', width: '100%' }}>
          {/* Left */}
          <div style={{ paddingRight: 60, borderRight: '1px solid rgba(180,140,60,0.2)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 32 }}>
              <div style={{ width: 40, height: 1, background: '#b48c3c' }} />
              <span style={{ fontSize: 10, letterSpacing: 5, textTransform: 'uppercase', color: '#b48c3c' }}>Desde 2018 · Trujillo</span>
            </div>
            <h1 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 'clamp(52px,6vw,88px)', fontWeight: 300, lineHeight: 1, color: '#f0e6cc', marginBottom: 8, letterSpacing: -1 }}>
              La Perfección<br />
          ...
          {/* Right */}
          <div style={{ paddingLeft: 60, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1, background: 'rgba(180,140,60,0.2)', marginBottom: 40 }}>
              {[['12+', 'Cortes Premium'], ['7', 'Años de Fuego'], ['48', 'Mesas Privadas'], ['★ 5', 'Experiencia']].map(([num, label]) => (
                <div key={label} style={{ background: '#080706', padding: '28px 24px' }}>
                  <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 42, fontWeight: 300, color: '#b48c3c', lineHeight: 1, display: 'block', marginBottom: 6 }}>{num}</span>
                  <span style={{ fontSize: 9, letterSpacing: 3, textTransform: 'uppercase', color: '#6b5c40' }}>{label}</span>
                </div>
              ))}
            </div>
            <p style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: 'italic', fontSize: 17, color: '#8a7558', lineHeight: 1.7, borderLeft: '2px solid rgba(180,140,60,0.4)', paddingLeft: 20 }}>
              "El fuego revela la esencia de cada corte — nosotros solo sabemos escucharlo."
            </p>
          </div>
        </div>
      </header>
      ...
      {/* PHILOSOPHY */}
      <section style={{ padding: '120px 80px', maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 80, alignItems: 'center' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 80, fontWeight: 300, color: 'rgba(180,140,60,0.2)', lineHeight: 1 }}>01</div>
          <div style={{ fontSize: 10, letterSpacing: 5, textTransform: 'uppercase', color: '#b48c3c', writingMode: 'vertical-rl', transform: 'rotate(180deg)', whiteSpace: 'nowrap' }}>Nuestra Filosofía</div>
        </div>
        <p style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 'clamp(28px,3vw,42px)', fontWeight: 300, fontStyle: 'italic', lineHeight: 1.5, color: '#c8b48a' }}>
          El fuego es nuestro lenguaje. Cada corte es una{' '}
          <span style={{ color: '#f0e6cc' }}>obra en proceso</span>, trabajada con precisión y respeto absoluto por el producto. No cocinamos:{' '}
      ...
      <section style={{ padding: '0 48px 100px' }}>
        {/* Section header */}
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 32, maxWidth: 1200, margin: '0 auto 60px', padding: '0 32px' }}>
          ...
          <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 'clamp(32px,4vw,52px)', fontWeight: 300, color: '#f0e6cc', letterSpacing: -0.5 }}>Arte & Servicio</h2>
          <div style={{ flex: 1, height: 1, background: 'rgba(180,140,60,0.2)', marginBottom: 8 }} />
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr', gap: 2, maxWidth: 1200, margin: '0 auto' }}>
          {[
            ...
            <div key={card.name} className="fr-arte-card" style={{ position: 'relative', overflow: 'hidden', background: '#111009', cursor: 'pointer' }}>
              ...
              <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '40px 28px 28px', background: 'linear-gradient(to top,rgba(8,7,6,0.95) 0%,transparent 100%)' }}>
      ...
      {/* MENU */}
      <section style={{ padding: '100px 0', background: '#050403', borderTop: '1px solid rgba(180,140,60,0.15)', borderBottom: '1px solid rgba(180,140,60,0.15)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 80px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 32, marginBottom: 48 }}>
            ...
            <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 'clamp(32px,4vw,52px)', fontWeight: 300, color: '#f0e6cc', letterSpacing: -0.5 }}>Selección Premium</h2>
            <div style={{ flex: 1, height: 1, background: 'rgba(180,140,60,0.2)', marginBottom: 8 }} />
          </div>

          {/* Tabs */}
          <div style={{ display: 'flex', borderBottom: '1px solid rgba(180,140,60,0.2)', marginBottom: 60 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 2 }}>
            {menuData[activeTab].map((item, i) =>
              item.name ? (
                ...
                <div key={i} style={{ background: '#070504' }}>
                  <div style={{ padding: '48px 28px', display: 'flex', flexDirection: 'column', justifyContent: 'center', height: '100%', minHeight: 340 }}>
                    <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 18, color: '#6b5c40', fontStyle: 'italic', marginBottom: 20 }}>{item.desc}</div>
                  </div>
                </div>
              )
            )}
      ...
      {/* EXPERIENCE STRIP */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', borderTop: '1px solid rgba(180,140,60,0.15)' }}>
        {[
          {
            num: 'I', title: 'Leña Seleccionada',
            desc: 'Trabajamos exclusivamente con madera de quebracho y espino para lograr el calor perfecto.',
          ...
          {
            num: 'II', title: 'Maduración Propia',
            desc: 'Nuestros cortes reposan entre 21 y 45 días en cámaras de temperatura controlada.',
          ...
          {
            num: 'III', title: 'Servicio de Autor',
            desc: 'Cada visita es irrepetible. Nuestro equipo diseña experiencias gastronómicas personalizadas.',
          ...
          <div key={block.num} className="fr-exp-block" style={{ padding: '60px 48px', borderRight: i < 2 ? '1px solid rgba(180,140,60,0.15)' : 'none', position: 'relative', overflow: 'hidden' }}>
            ...
            <p style={{ fontSize: 11, color: '#6b5c40', lineHeight: 2, letterSpacing: 0.5, maxWidth: 260 }}>{block.desc}</p>
          </div>
        ))}
      </div>
      ...
      <section style={{ padding: '120px 80px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 70% 80% at 50% 50%,rgba(120,70,10,0.12) 0%,transparent 70%)' }} />
        ...
        <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 'clamp(40px,5vw,72px)', fontWeight: 300, color: '#f0e6cc', lineHeight: 1.1, marginBottom: 16 }}>
      ...
      {/* FOOTER */}
      <footer style={{ borderTop: '1px solid rgba(180,140,60,0.2)', padding: '60px 80px 40px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 60, background: '#040302' }}>
        <div>
          <div style={{ fontFamily: "'Josefin Sans',sans-serif", fontWeight: 100, fontSize: 20, letterSpacing: 8, textTransform: 'uppercase', color: '#e8d5a3', marginBottom: 12 }}>Capris Bull</div>
        ...
        <div>
          ...
          {['Av. Juan Pablo II, Trujillo, Perú', '+51 999 000 000', 'hola@caprisbull.pe'].map(t => (
      ...
      <div style={{ borderTop: '1px solid rgba(180,140,60,0.1)', padding: '24px 80px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#040302' }}>
        <span style={{ fontSize: 10, color: '#3d3425', letterSpacing: 2 }}>© 2026 Capris Bull · Todos los derechos reservados</span>
